...
(function ($) {
    ...
    var swatchColor = function () {
        if ($(".card-product").length > 0) {
            // Lắng nghe cả sự kiện click và hover
            $(".color-swatch").on("click mouseover", function () {
                // Lấy ảnh của màu đang hover/click
                var swatchColor = $(this).find("img.image-product").attr("src"); // main_image
                var swatchColorHover = $(this).find("img.hover-image-product").attr("src"); // hover_image
                console.log(swatchColor);
                console.log(swatchColorHover);
                // Cập nhật ảnh chính
                var imgProduct = $(this).closest(".card-product").find(".img-product");
                imgProduct.attr("src", swatchColor);

                // Cập nhật ảnh hover
                var imgHover = $(this).closest(".card-product").find(".img-hover");
                imgHover.attr("src", swatchColorHover);
