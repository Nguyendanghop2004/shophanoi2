    ...
    <div class="card card-primary">
        <div class="card-header">
            ...
            <div class="card-header-action">
                <a href="{{ route('tag_materials.create') }}" class="btn btn-primary">Thêm Chất Liệu</a>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="card-header-action">
                    <form class="form-inline" method="GET" action="{{ route('tag_materials.index') }}">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @if($tagMaterials->isEmpty())
                            <tr>
                                <td colspan="3" class="text-center text-danger">Không có chất liệu nào.</td>
                            </tr>
                        @else
                            @foreach ($tagMaterials as $index => $tagMaterial)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $tagMaterial->name }}</td>
                                    <td>
                                        <div class="d-flex justify-content-start">
                                            <a href="{{ route('tag_materials.edit', $tagMaterial->id) }}" class="btn btn-warning ml-2"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('tag_materials.destroy', $tagMaterial->id) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Bạn có chắc chắn muốn xóa?')">
                                                @csrf
                                                @method('DELETE')
